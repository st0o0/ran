...
func (t *IRCTrap) Start(ctx context.Context) error {
	ln, err := ListenMultiTCP(ctx, t.cfg.TrapAddrs("irc"), t.cfg.ProxyProtocol)
	if err != nil {
		return fmt.Errorf("irc listen: %w", err)
	}
	t.listener = ln
	t.logger.Info("listening", "addrs", t.cfg.TrapAddrs("irc"))
	...
	for {
		...
		if err != nil {
			...
			LogErrorStandalone(t.logger, "irc", "accept_failed", err)
			continue
		}
		t.wg.Add(1)
...
func (t *IRCTrap) handle(ctx context.Context, conn net.Conn) {
	...
	sess := NewSession("irc", "tcp", host, port, destPort, t.logger)

	if !t.limiter.Acquire(host) {
		LogRejected(t.logger, "irc", "tcp", destPort, host, "rate_limit")
		return
	}
