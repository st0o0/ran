...
- **THEN** the delays before each response SHALL be: attempt 0 = 2s, attempt 1 = 4s, attempt 2 = 8s, attempt 3 = 8s (capped at 4x base)
...
#### Scenario: Delay capped at 4x base
- **WHEN** `RAN_AUTH_DELAY=2s` is set and a client reaches attempt 5
- **THEN** the delay SHALL remain at 8s (4 x 2s), not escalate further
